...
private fun checkCapacity(capacity: Int) {
    when (capacity) {
        ...
        Channel.CONFLATED -> return
            ...
            // Everything else than UNLIMITED or CONFLATED does not guarantee that channel.offer()
            // succeeds all the time. We don't support these use cases for now
            throw IllegalArgumentException(
                "Bad channel capacity ($capacity). Only UNLIMITED and CONFLATED are supported"
            )
    }
}
 ...
 * @param capacity the {@link Capacity} used for the underlying channel.
 * Only {@link kotlinx.coroutines.channels.Channel.UNLIMITED}
